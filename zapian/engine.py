...
def replace_document(site_name, catalog_name, part_name, uid, doc, flush=True, **kw):
    """ 重建文档索引 """
    IR.replace_document(site_name, catalog_name, part_name, uid,
            process_doc(catalog_name, doc), flush, **kw)

def delete_document(site_name, catalog_name, part_name, uids, flush=True, **kw):
    """ 删除一个文档 """
...
def update_document(site_name, catalog_name, part_name, uid, doc, flush=True, **kw):
    """ 更改文档某个字段的索引
    注意，xapian不支持，这里手工处理 """
...
def flush(site_name, catalog_name, part_name, **kw):
    """ """
    IR.commit(site_name, catalog_name, part_name, **kw)

# 搜索
def search(site_name, catalog_name, parts, query_set):
    """ 搜索查询结果 """
